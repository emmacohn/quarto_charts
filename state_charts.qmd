...
job_long <- jobcomp |> 
  select(geo_name, covid_change, trump_change) |> 
  pivot_longer(cols = c(covid_change, trump_change),
               names_to = "comparison",
               values_to = "change") |> 
  mutate(percents = ifelse(change < 0, "negative", "positive"))
```
...
for(i in seq_along(states)) {
  ...
  plot <- job_long |> 
    ...
    ggplot(aes(x = comparison, y = change, fill = percents)) +
    geom_col() +
