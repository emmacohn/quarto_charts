---
title: "Unemployment rate change by state"
format: 
  html:
...
jobchange <- read_csv("/projects/ecohn/state_jobs_day/output/lf_unemployment.csv") |> 
filter(!startsWith(series_id, "LASRD")) |> 
select(-series_id)
...
jobcomp <- jobchange |>
  ...
  mutate(feb_2020 = .data[["2020-02-01"]],
         ...
         covid_change = (.data[[current_month]] - feb_2020)*100,
         trump_change = (.data[[current_month]] - jan_2025)*100,
...
```{r plots, echo=FALSE, warning=FALSE, fig.width=7, fig.height=5, results='asis'}
...
for(i in seq_along(states)) {
  ...
  plot <- job_long |> 
    ...
    scale_fill_manual(values = c("us" = "#d73027",
                                 "state" = "#4575b4")) +
    labs(title = paste(state, "- Percentage point change in unemployment rate"),
         y = "Urate percentage point change",
        fill = NULL) +
